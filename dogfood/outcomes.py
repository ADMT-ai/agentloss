...
def human_queue(stream_by_no, oracle):
    ...
    for key, d in agentloss.STORE.decisions.items():
        if d.action in ("hold", "reject"):
            truth = oracle[key]["correct_action"]
            agentloss.report_outcome(key, ground_truth=truth, source="human_queue", fidelity="gold")


def recovery_audit(oracle, cfg, rng):
    """Delayed catch of a fraction of true escapes → gold + realized loss."""
    for key, d in list(agentloss.STORE.decisions.items()):
        if d.action != "approve":
            continue
        truth = oracle[key]
