# AgentProof SDK Spec — Agent-Adoptable Reliability Instrumentation

_Working name: **AgentProof** (placeholder). OTel namespace: `ailoss.*`._
...
```python
from agentproof import instrument, decision, Decision
...
`agentproof doctor --json` — the machine-readable check a coding agent runs to confirm
integration:
...
Invoice ─▶ [ agent + @decision ]──tool calls──▶ ERP, payment rail
                 ...
                 ▼
        agentproof processor (in customer VPC)
...
activated (AGENTPROOF_KEY set)     ← one env var; agent scaffolds it, human authorizes
...
- **MCP server** (`agentproof-mcp`): `how_to_instrument(framework)`,
...
- **Framework plugins:** `agentproof-langgraph`, `agentproof-crewai` (auto-wrap tool nodes).
