...
Most observability tools score *quality* (LLM-judge, hallucination rate). AgentProof closes the
...
```python
from agentproof import decision, report_outcome, Decision
...
checks that AgentProof recovers the true error rate and dollar loss:

```bash
python -m dogfood.run                                  # deterministic mock, no deps
AGENTPROOF_VERIFIER_LLM=claude ANTHROPIC_API_KEY=... python -m dogfood.run
```

## For AI coding agents

AgentProof is built to be discovered and wired by coding agents:
[`llms.txt`](llms.txt), the [`instrument-agent-reliability`](skills/instrument-agent-reliability/SKILL.md)
skill, the [`AGENTS.md`](AGENTS.md) rule, and an [MCP server](mcp/agentproof_mcp.py)
